...
import {ownersLinks} from '../constants';
      ...
      <div>
        <nav className='w-full flex py-6 justify-between items-center navbar'>
          ...
          <ul className='list-none sm:flex hidden justify-end items-center flex-1 mr-10'>
            { ownersLinks.map((nav, index) =>(
              <li 
                key={nav.id} className={`font-poppins font-normal cursor-pointer hover:text-secondary text-[16px] ${index === ownersLinks.length-1 ? 'mr-0' : 'mr-20'} active:text-secondary text-black`}
              >
                <Link
                    to={nav.id}
                    className={` ${activeLink === nav.id ? activeStyle : inactiveStyle} hover:text-secondary`}
          ...
          <div className="sm:hidden flex flex-1 justify-end items-center">
            ...
            <div className={`${toggle ? 'flex' :'hidden'} flex flex-col p-6 bg-black-gradient absolute top-10 right-0 mx-4 my-2 min-w-[140px] rounded-xl side-bar`}>
              ...
              { ownersLinks.map((nav, index) =>(
                <li 
                  key={nav.id} className={`font-poppins font-normal cursor-pointer text-[16px] ${index === ownersLinks.length-1 ? 'mr-0' : 'mb-10'} `}
                >
                  <Link
                    to={nav.id}
                    className={` ${activeLink === nav.id ? activeStyle_ss : inactiveStyle_ss}`}
                    onClick={() => handleLinkClick(nav.id)}
                  >
                    {nav.title}
                  </Link>
                ...
                <form action="login" method="">
                  <button id="meta-btn" className={` mt-10`} onClick={onDisconnected} >
