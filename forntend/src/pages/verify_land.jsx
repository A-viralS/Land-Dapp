...
// export default verify_land
  ...
  const verify_land = ({ state }) => {
    ...
    const tableRows = landList.map((land) => (
      <tr key={Math.random()} className="py-5">
        <td className="text-white text-center">{toSentenceCase(land.district)}</td>
        <td className="text-white text-center">{toSentenceCase(land.city)}</td>
        <td className="text-white text-center">{toSentenceCase(land.state)}</td>
        <td className="text-white text-center">{toSentenceCase(land.propertyNumber)}</td>
        <td className="text-white text-center">{toSentenceCase(land.size.toString())} Square Feet</td>
        <td className="text-white text-center">{toSentenceCase(land.landDocument)}</td>
        <td className="text-white text-center">
          <button
            id="landid"
            className="add-btn focus:bg-blue-300 mr-10 focus:text-black text-white pb-5"
    ...
    return (
      <div className="flex flex-col relative bg-primary w-full overflow-hidden h-full">
        <Topnav />
        <div className=" min-h-[77.5vh]">
          <div className=" flex justify-center flex-col items-center relative pt-[50px]">
            ...
            {landList.length === 0 ? (
              ...
              <table class="w-[80%] mx-auto mt-10 rounded-md border border-opacity-50  border-white z-[50]">
                <thead className="text-gradient">
                  <tr className="mt-5">
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">District</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">City</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">State</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">Property Number</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">Size</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">Land Document</th>
                    <th className="bg-black-600 text-center py-4 mx-2 border-b-sky-700 border-b  ">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {tableRows}
          ...
          <div class="w-[200px] h-[200px]">
            <div class="absolute z-[1] w-[20%] h-[25%] right-0 top-0 left-30 pink__gradient"></div>
            <div class="absolute z-[3] w-[40%] h-[40%] right-0 top-0 white__gradient"></div>
            <div class="absolute z-[2] w-[20%] h-[20%] right-0 top-0 blue__gradient"></div>
